...
if (filter == "fragile") result = result.Where(c => c.Tires.Any(t => t.Pressure < 1));
else if (filter == "flammable") result = result.Where(c => c.Engine.Power > 250);

foreach (Car car in result) Console.WriteLine(car.Model);